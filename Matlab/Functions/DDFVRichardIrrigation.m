function [solution, Erreur]=DDFVRichardIrrigation(psi_old,J)
    ...
    psi_current = psi_old;
    D = r*[2*10^(-6), 0; 0, 5*10^(-5)];% Tenseur de permeabilite
    ...
    fprintf('Temps Irrigation: %d', Tmax);
    ETo=CalculEvapotranspirationJournaliere(J); %J : jour julien
    Tpot=TranspirationPotentielle(ETo);% Calcul de la transpiration potentielle.
